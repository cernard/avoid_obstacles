...
void atk_ms53l1m_uart_init(uint32_t baudrate)
{
    ...
    HAL_UART_Init(&g_uart_handle);                                  /* 使能ATK-ESP8266 UART
                                                                     ...
                                                                     * 该函数定义在文件usart.c中
                                                                     */
//    HAL_UART_Receive_IT(&g_uart_handle, g_uart_rx_frame.buf, ATK_MS53L1M_UART_RX_BUF_SIZE); /* 使能ATK-MS53L1M UART接收中断 */
}
...
//        HAL_UART_Receive_IT(&g_uart_handle, &tmp, 1);                       /* UART接收数据 */
